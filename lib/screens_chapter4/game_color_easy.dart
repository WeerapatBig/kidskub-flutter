...
class GameColorEasyScreen extends StatefulWidget {
  const GameColorEasyScreen({super.key});

  @override
  State<GameColorEasyScreen> createState() => _GameColorEasyScreenState();
}
...
class _GameColorEasyScreenState extends State<GameColorEasyScreen> {
  ...
  AnswerResult? _answerResult; // null = ไม่ได้ตอบอะไรล่าสุด
...
// เมื่อได้รับ onGameOver จาก ColorGame => แสดง ResultWidget
  ...
  void _onTargetColorChanged(Color newColor) {
    setState(() {
      _spawnNewTarget(newColor);
      // อัปเดตในตัว levelData ก็ได้ หรือจะปล่อยให้ ColorGame ทำแล้ว
      // เราแค่สั่ง setState() เพื่อให้ Container rebuild
      levelData.targetColor = [newColor];
    });
  }

  void _onAnswerResult(bool isCorrect) {
    // ได้คำตอบถูก/ผิดจาก GameLogic
    setState(() {
      _answerResult = isCorrect ? AnswerResult.correct : AnswerResult.wrong;
    });
    ...
    Future.delayed(const Duration(milliseconds: 500), () {
      ...
      // หลังหน่วง: ซ่อนสัญลักษณ์ถูก/ผิด
      setState(() {
        _answerResult = null;
      });
    });
    ...
    return
      ...
      child: Stack(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  color: levelData.targetColor.first,
                  border: Border.all(color: Colors.black, width: 6),
                ),
              ),
            ),
          ...
          // 2) ถ้าเพิ่งตอบถูก => แสดงภาพ check.png
          if (_answerResult == AnswerResult.correct)
            Positioned(
          ...
          // 3) ถ้าเพิ่งตอบผิด => แสดงภาพ cross.png
